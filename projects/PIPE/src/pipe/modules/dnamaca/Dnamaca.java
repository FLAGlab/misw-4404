...
/* wjk - recommend to keep this module for reference only */

public class Dnamaca implements Module {
	  private static final String MODULE_NAME = "DNAmaca (Not ready)";
	...
	ActionListener runDnamacaAction=new ActionListener() {
		public void actionPerformed(ActionEvent arg0) {
			...
			} else if (pnmldata.getPlacesCount() == 0) {
...
//		performance();
	...
	private boolean hasTimed() {
		for(int i=0; i<transitions.length; i++) {
			if (transitions[i].isTimed()) return true;
		}
		return false;
	}
