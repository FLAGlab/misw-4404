...
public class Simulation implements Module {
  private static final String MODULE_NAME = "Simulation (Not ready)";
  ...
  public void run(DataLayer pnmlData) {
    ...
    settings.setBorder(new TitledBorder(new EtchedBorder(),"Simulation parameters"));
  ...
  String simulate(DataLayer data,int cycles,int firings) {
    ...
    for(i=0; i < cycles; i++) {
      ...
      // Changed by David Patterson April 24, 2007
    ...
    for(i=0; i < marking.length; i++) {
      ...
      for(j=0; j < cycles ;j++)
        //Sum of squares
        errorResult[i] = errorResult[i] + (overallAverages[j][i] - avgResult[i])*(overallAverages[j][i] - avgResult[i]);
    ...
    if(averageTokens != null && errorResult != null && averageTokens.length > 0 && errorResult.length > 0) {  // Write table of results
      results.add("Place");
