...
public class InvariantAnalysis implements Module {
  ...
  /**
   * Invariant analysis was originally only performed on the initial markup matrix.
   * This is not what the user expects however as when changes are made to the petri
   * net, the invariant analysis then does not change to reflect this. The method
   * calls have been changed to pass the current markup matrix as the parameter for
   * invariant analysis.
   * 
  ...
  public String findNetInvariants(int[] M){
    return reportTInvariants(M)+reportPInvariants(M);
  }
  ...
  public String reportPInvariants(int[] M){
    ...
    return result+findPEquations(M);
  }
  ...
  public String reportTInvariants(int[] M){
    TInvariants = findVectors(new PNMatrix(IncidenceMatrix.getArrayCopy()));
    ...
    while (!(C.isZeroMatrix())) {
      if (C.checkCase11()){
        ...
        while ((h = C.firstNonZeroRowIndex()) > -1) {
          ...
          while (!(isEmptySet(chj))){ // chj empty only when there is just one nonzero element in the whole row, this should not happen as this case is eliminated in the first step, so we would never arrive at this while() with just one nonzero element
...
//--------------------------------------------------------------------------------------
      ...
      if (pPlusLength != 0){ // set of positive coef. indices must me non-empty
        ...
        for (int j = 0; j < pPlusLength; j++){
          for (int k = 0; k < pMinusLength; k++){
            int jC = pPlus[j]-1, kC = pMinus[k]-1; // coefficients of row h, cols indexed j, k in pPlus, pMinus respectively
            // find coeficients for linear combination, just the abs values of elements
            // this is more efficient than finding the least common multiple and it does not matter
            // since later we will find gcd of col and we will normalise with that the col elements
            int a = -B.get(h, kC), b = B.get(h, jC);

            // create the linear combination a*jC-column + b*kC-column, an IntMatrix mx1 where m = number of rows of B
            m = B.getRowDimension();
    ...
    // k is the index of column to be eliminated, if it is -1 then there is no col to be eliminated
    int k = 0;
   ...
   * union. The resulting array size is the same as the 2 arrays and they are both
   * equal. We are only interested in non-zero elements. One of the 2 input arrays
