 ...
 * @author Nadeem
 * 
 * This class is always used as a static class. It is used
 * to determine the steady state of a general stochastic 
 * petri net. The only public method is solve() which
 * calculates the steady state.
 */
public class SteadyStateSolver {
	
	public SteadyStateSolver(){
		// This class should never be instantiated.
	}
	
	public static double[] solve(File rgfile) throws ImmediateAbortException{
		
		// A sparse matrix containing the transition probabilities between tangible states.
	...
	/**
	 * createSparseMatrix()
	 * Read's the state to state transitions from a reachability
	 * graph file and the associated rates and creates the matrix
	 * Q. However, as it is a sparse matrix (lots of zeroes) it
	 * creates it in a more efficient form than an n by n array.
	 * Instead it returns an array of LinkedLists. Each ROW of 
	 * the array represents a COLUMN of the matrix. Each object
	 * in the linked list represents an element of the matrix
	 * as a record <row, rate>. This way, only non-zero elements
	 * are stored. It is stored column wise as later calculations
	...
	private static LinkedList[] createSparseMatrix(File rgfile) throws ImmediateAbortException{
		...
		numcolumns = rgheader.getNumStates();
		...
		try{
			// Populate the sparse matrix
			// We don't need to insert elements in a column in row order
			// as each element tells you which row it's in. We just need
			// to get the column right.
			...
			for(int record = 0; record < numtransitions; record++){
				...
				System.out.print(f.format(((double)record/numtransitions)*100) + "% complete.  \r");
			}
			
			// Now add in the diagonal elements. Each diagonal
			// element is the negative sum of the off diagonals
			// on that ROW.
	...
	/**
	 * gaussSiedel()
	 * Solves matrix equations of the form Ax = b for x
	 * using the Gauss-Siedel iterative method. In this
	 * case, A is a sparse matrix, b=0 and the sum of
	 * all xi = 1
	 * 
	...
	private static double[] gaussSiedel(LinkedList[] A){
		...
		while(!plausible){
			...
			// Now we've been through an iteration on all the rows,
			// check the residuals to see how accurate our answer is
			for(int row = 0; row < numrows; row++){
				iterator = A[row].iterator();
				residual = 0;
				while(iterator.hasNext()){
			...
			// If we think all the rows have converged, we should
			// check whether the steady state values are plausible
			// by checking none of the values are < 0
 ...
 * element in the matrix as a pair of values <row, rate>.
 */
class MatrixElement{
	private int row;
	private double rate;
