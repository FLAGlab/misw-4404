 ...
 * transitions in the animation history. These actions are no longer allowed after the fix.
 *  
 * */
public class AnimationHistory extends JTextPane {
   ...
   public AnimationHistory(String text)
      ...
      //doc.insertString(doc.getLength(),"Initial Marking\n",regular);
      fSeq = new Vector();
      fSeq.add("Initial Marking");  //Pere
      currentItem = 1;
      updateText();                 //Pere
   }
   ...
   /* Pere */
   public boolean isStepForwardOK(){
      return currentItem < fSeq.size();
   }
   
   
   /* Pere */
   public boolean isStepBackOK(){
