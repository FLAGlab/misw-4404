...
public class StateList {
	 ...
	 * StateList()
	 * 
	 * @param rgfile		A reachability graph file containing a list of
	 * 						tangible states.
	 * @throws IOException
	 * @throws StateSpaceTooBigException
	 * 
	 * @author Edwin Chung Mar 2007 - modified the constructor so it can allow the 
	 * reading of the type of state i.e. whether it is tangible or not  
	 */
	public StateList(File rgfile, boolean readTangible) throws IOException, StateSpaceTooBigException{
		...
		try{
			for(state=0; state < numtangiblestates; state++){
				if (readTangible) currentrecord.read1(rgheader.getStateArraySize(), input);
				else currentrecord.read(rgheader.getStateArraySize(), input);
	 ...
	 * If the constructor StateList(RandomAccessFile rgfile) has been used,
	 * then this method can be called as the file should contain no
	 * duplicate entries. This makes the code more efficient and should
	 * execute faster as there could potentially be a few hundred thousand
	 * states.
	 * 
