 ...
 * @author Matthew Worthington -- changed the ModuleManger to dynamically load all class files in the 
 * module directory without the need to update the cfg files and provide path properties. Modules can 
 * now be dropped into the module folder and automatically loaded on all subsequent executions of pipe. 
 * Also refactored to reduce number of methods loaded with reflection into the Jtree which were 
 * subsequently never used. Now only loading the run method of each of the modules. (Jan,2007)
 */
...
//	public CreateGui gui;
	...
	public File getModuleDir() {
		...
		File modLocation = new File(ExtFileManager.getClassRoot(this.getClass()),
				System.getProperty("file.separator") + "pipe"+ System.getProperty("file.separator")+"modules");
	...
	{
		final ExtensionFilter class_files=new ExtensionFilter(CLASS_FILE_EXTENSION, CLASS_FILE_DESC);
		...
		for (int a = 0 ; a < children.length ; a++) {
			...
			{	
				classes.addAll(getModuleClasses(children[a]));
			}
			else  if (class_files.accept(children[a])) 
			{

				aModuleClass = ModuleLoader.importModule(children[a]);
	 ...
	 *  This consists of two nodes, one resposible for listing
	 *  all the available modules from the module directory,
	 *  and another for admin options such as list refreshing.
	 *  Each node of the tree has it's own user object, for class
	 *  nodes this will be ModuleClass, for method nodes ModuleMethod,
	 *  and another one yet to be implemented for other options.
	 *  When the user clicks on a method node the method is invoked.
	 *  
	 *  Matthew - modified to reduce unnecessary reflection, now only 
	 *  loading the run method of each module class into the tree
	 */


	//only add the run method to the tree
	private void addClassToTree(Class moduleClass) {
		...
		if (installedModules.add(moduleClass)) {
			...
			try {
				...
				m.setName(modNode.getUserObject().toString()); // rename "run" to parent name
	...
	public JTree getModuleTree() {
		...
		Vector names = getModuleClasses(dir);
	...
	public void addModuleToTree(DefaultMutableTreeNode newNode) {
//		treeModel.insertNodeInto(newNode,load_modules,load_modules.getChildCount());
//		moduleTree.expandPath(moduleTree.getPathForRow(1));
	}
	...
	class RemoveModuleAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(o instanceof ModuleMethod) installedModules.remove(((ModuleMethod)o).getModClass());
			else if(o instanceof ModuleClassContainer) installedModules.remove(((ModuleClassContainer)o).returnClass());
			else System.err.println("Don't know how to delete class for "+o.getClass()); 
			...
			if(selRow != -1) {
				...
				if(e.getClickCount() == 2)	{
					if (nodeObj instanceof ModuleMethod) {
						...
						if(parameter!=null)((ModuleMethod)nodeObj).execute(parameter);
					} else if (nodeObj == loadNodeString) {
						// DefaultMutableTreeNode newNode ;

						//Create a file chooser
						JFileChooser fc = new JFileChooser();
						fc.setFileFilter(new ExtensionFilter(CLASS_FILE_EXTENSION,CLASS_FILE_DESC));
						//In response to a button click:
						int returnVal = fc.showOpenDialog(parent);
						File moduleProp;
						Class newModuleClass;
						if(returnVal == JFileChooser.APPROVE_OPTION) {
							moduleProp = fc.getSelectedFile();
							newModuleClass  = ModuleLoader.importModule(moduleProp);
